import XCTest
@testable import Y2020D14

class SolutionTests: XCTestCase {
    func test_solve1_withSampleInput_shouldReturn165() {
        let path = Bundle(for: type(of: self)).path(forResource: "sample1", ofType: "txt")!
    ...
    func test_solve2_withSampleInput_shouldReturn208() {
        let path = Bundle(for: type(of: self)).path(forResource: "sample2", ofType: "txt")!
        let input = try! String(contentsOfFile: path, encoding: .utf8)
